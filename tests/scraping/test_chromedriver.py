...
from lox_services.scraping.selenium_util import wait_for_end_of_download, wait_until_clickable_and_click
class TestChromeDriver(unittest.TestCase):

    def setUp(self):
        self.folder_path = os.path.join(os.getcwd(), 'tests', 'scraping', 'download_folder')
    ...
    def test_run_chromedriver(self):
        ...
        self.assertEqual(sum(len(files) for _, _, files in os.walk(self.folder_path)), 0)
        ...
        self.assertEqual(sum(len(files) for _, _, files in os.walk(self.folder_path)), 1)
