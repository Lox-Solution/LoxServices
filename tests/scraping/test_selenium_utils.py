...
class TestUtils(unittest.TestCase):
    ...
    def clear_cookie_banner(self, wait):
        ...
        wait_until_clickable_and_click(wait, '#qc-cmp2-ui > div.qc-cmp2-footer.qc-cmp2-footer-overlay.qc-cmp2-footer-scrolled > div > button.sc-ifAKCX.ljEJIv', DEFAULT_TIMEOUT, DEFAULT_SELECTOR_TYPE)
        time.sleep(0.5)
    
    def ups_clear_cookie_banner(self, wait):
        wait_until_page_loaded(driver = self.driver)
        # Clear the cookie banner
        wait_until_clickable_and_click(wait, '#__tealiumGDPRecModal > div.privacy_prompt_middle > div > div.privacy_prompt_content > div.option_set > div:nth-child(1) > label', DEFAULT_TIMEOUT, DEFAULT_SELECTOR_TYPE)
        wait_until_clickable_and_click(wait, '#consent_prompt_submit', DEFAULT_TIMEOUT, DEFAULT_SELECTOR_TYPE)
        time.sleep(0.5)
    ...
    def test_safe_find_element(self):
        element = safe_find_element(driver = self.driver, selector_type= DEFAULT_SELECTOR_TYPE, selector=DEFAULT_RIGHT_CSS_SELECTOR, wait = self.wait, timeout = DEFAULT_TIMEOUT)
        self.assertIsInstance(element, WebElement)

        with self.assertRaises(TimeoutException):
            safe_find_element(driver = self.driver, selector_type= DEFAULT_SELECTOR_TYPE, selector=DEFAULT_WRONG_CSS_SELECTOR, wait = self.wait, timeout = DEFAULT_TIMEOUT)
    ...
    def test_safe_find_elements(self):
        elements = safe_find_elements(driver = self.driver, selector_type=DEFAULT_SELECTOR_TYPE, selector='#commonWebElements > form', timeout=DEFAULT_TIMEOUT)
        self.assertEqual(len(elements), 2)

        with self.assertRaises(NoSuchElementException):
            safe_find_elements(driver = self.driver, selector_type=DEFAULT_SELECTOR_TYPE, selector=DEFAULT_WRONG_CSS_SELECTOR, timeout=DEFAULT_TIMEOUT)
        

    def test_wait_until_clickable_and_click(self):
        self.clear_cookie_banner(self.wait)
        # Test case for successful click with wait
        wait_until_clickable_and_click(self.wait, DEFAULT_RIGHT_CSS_SELECTOR, DEFAULT_TIMEOUT, DEFAULT_SELECTOR_TYPE)
        
        # If the function completes without raising an exception, it executed well
        self.assertTrue(True)
        
        # Test case for successful click with driver
        wait_until_clickable_and_click(None, DEFAULT_RIGHT_CSS_SELECTOR, DEFAULT_TIMEOUT, DEFAULT_SELECTOR_TYPE, self.driver)
        ...
        with self.assertRaises(TimeoutException):
            wait_until_clickable_and_click(self.wait, DEFAULT_WRONG_CSS_SELECTOR, DEFAULT_TIMEOUT, DEFAULT_SELECTOR_TYPE)
            
        # Test case for click on non-clickable element and driver
        with self.assertRaises(TimeoutException):
            wait_until_clickable_and_click(None, DEFAULT_WRONG_CSS_SELECTOR, DEFAULT_TIMEOUT, DEFAULT_SELECTOR_TYPE, self.driver)
    ...
    def test_wait_then_clear(self):
        ...
        safe_send_keys(self.driver, DEFAULT_RIGHT_CSS_SELECTOR, DEFAULT_TEXT_TO_WRITE, selector_type=By.CSS_SELECTOR)
    ...
    def test_wait_then_send_keys(self):
        self.driver.get('https://wwwapps.ups.com/doapp/signup?loc=nl_NL&ClientId=13&returnto=https:%2F%2Fwww.ups.com%2Fnl%2Fnl%2FHome.page')
        self.ups_clear_cookie_banner(self.wait)
        # Test case when the element exists
        wait_then_send_keys(self.driver, self.wait, '#signUpName', DEFAULT_TEXT_TO_WRITE, clear=True)
        ...
        wait_then_send_keys(self.driver, self.wait, '#signUpName', DEFAULT_TEXT_TO_WRITE, clear=False)
        ...
        # Test case when the element doesn't exist
        with self.assertRaises(TimeoutException):
            wait_then_send_keys(driver=self.driver,wait=self.wait,selector= DEFAULT_WRONG_CSS_SELECTOR, input_text=DEFAULT_TEXT_TO_WRITE)
    ...
    def test_safe_send_keys(self):
        ...
        # Test case when the element doesn't exist
        with self.assertRaises(TimeoutException):
            safe_send_keys(self.driver, DEFAULT_WRONG_CSS_SELECTOR, DEFAULT_TEXT_TO_WRITE, selector_type=By.CSS_SELECTOR)

    def test_wait_till_disapear(self):
        self.driver.get('https://wwwapps.ups.com/doapp/signup?loc=nl_NL&ClientId=13&returnto=https:%2F%2Fwww.ups.com%2Fnl%2Fnl%2FHome.page')
        self.ups_clear_cookie_banner(self.wait)
        
        wait_till_disapear(wait=self.wait, selector_type=DEFAULT_SELECTOR_TYPE, selector="div > div > img", timeout=DEFAULT_TIMEOUT)
        self.assertTrue(True)
    ...
    def test_clear_storage(self):
        ...
        self.assertGreaterEqual(self.driver.execute_script("return window.localStorage.length;"), 2)
        clear_storage(self.driver, storage_type="local")
        self.assertEqual(self.driver.execute_script("return window.localStorage.length;"), 0)
        ...
        self.assertGreaterEqual(self.driver.execute_script("return window.sessionStorage.length;"), 2)
        clear_storage(self.driver, storage_type="session")
        self.assertEqual(self.driver.execute_script("return window.sessionStorage.length;"), 0)
        ...
        self.assertGreaterEqual(self.driver.execute_script("return window.localStorage.length;"), 1)
        self.assertGreaterEqual(self.driver.execute_script("return window.sessionStorage.length;"), 1)
