...
class UndetectedChromeDriver(undetected_chromedriver.Chrome):
    ...
    def __init__(self, *args, company: str, **kwargs):
        ...
        if platform in ["linux", "linux2"]:  # Linux (Ubuntu)
            ...
            raise Exception(
                "UndetectedChromeDriver exception. Unable to setup with current platform."
            )
...
def log_in_google_account(
    target_driver: webdriver.Chrome, username: str, password: str
):
    ...
    password_input = safe_find_element(
        target_driver, selector="#password > div > div > div > input"
    )
