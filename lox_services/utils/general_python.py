...
def is_file_or_folder_path(path: str) -> FileOrFolder:
    ...
    path = "".join(
        (
            c
            for c in unicodedata.normalize("NFD", path)
...
def string_search(
    ...
    *,
    ...
    right: bool = any(
        str(str_element) in str(string) for str_element in list_of_strings
    )
    if mode == "left":
        return left
    elif mode == "right":
...
def format_amount_to_human_string(value, language="EN", currency="€") -> str:
    ...
    try:
        if (
            value is None or value == "None" or value != value
        ):  # Check nan, None, and "None"
            return ""
        float(value)
        string = str(value)
        ...
        for high_index in range(len(splited_cents[0]), 0, -number_of_numbers_by_group):
            low_index = (
                high_index - number_of_numbers_by_group
                if high_index - number_of_numbers_by_group >= 0
                else 0
            )
...
def split_array(array: List[List], number_of_sub_arrays: int):
    ...
    return list(
        ...
        for i in range(number_of_sub_arrays)
    )


def split_date_range(
    start: datetime, end: datetime, intv: int, output_format: str = "%Y-%m-%d"
) -> Iterator[datetime]:
...
def remove_all_file_with_format_from_folder(
    *,
    ...
    if file_format not in ["csv", "xlsx", "pdf"]:
        raise ValueError(
            f"file_format has to be in ['csv', 'xlsx', 'pdf'] and it's '{file_format}'"
        )
...
def remove_extra_comma_dot_from_float(float_string: str) -> float:
    ...
    if last_comma > last_dot:
        float_formated_number = float_string[:last_comma].replace(",", "").replace(
            ".", ""
        ) + float_string[last_comma:].replace(",", ".")
    else:
        float_formated_number = (
            float_string[:last_dot].replace(",", "").replace(".", "")
            + float_string[last_dot:]
        )
